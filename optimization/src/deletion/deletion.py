...
class Deletion():
    ...
    def __get_violation_when_delete_one_tutorial_piece(
        ...
        self.__tutorial_occupation_array[
            student_index, teacher_index, tutorial_index, date_index, period_index] = 0
        [violation, _] = self.__cost_evaluator.violation_and_cost(self.__tutorial_occupation_array)
        self.__tutorial_occupation_array[
            student_index, teacher_index, tutorial_index, date_index, period_index] = 0
        return violation
    ...
    def __delete(self, student_index, teacher_index, tutorial_index, date_index, period_index):
        self.__tutorial_occupation_array[
            student_index, teacher_index, tutorial_index, date_index, period_index] = 0
    ...
    def __delete_one_tutorial_piece_if_improved(self):
        ...
        for order in range(self.__total_tutorial_piece_count):
            [student_index, teacher_index, tutorial_index, date_index, period_index] = \
                self.__tutorial_piece_evaluator.get_nth_tutorial_piece_indexes_from_worst(order)
            [violation_for_tutorial_piece, _] = \
                self.__tutorial_piece_evaluator.get_nth_tutorial_piece_violation_and_cost_from_worst(order)
            if self.__guard(student_index, teacher_index, tutorial_index, date_index, period_index): continue
            # 違反ウェイトを持たないコマはスキップする
            if violation_for_tutorial_piece == 0: continue
            violation_after = self.__get_violation_when_delete_one_tutorial_piece(
                student_index, teacher_index, tutorial_index, date_index, period_index)
            # 違反が減らない、もしくは変化しない場合は、スキップする
            if violation_after >= violation_before: continue
            self.__delete(student_index, teacher_index, tutorial_index, date_index, period_index)
            is_deleted = True
            break
        return is_deleted
