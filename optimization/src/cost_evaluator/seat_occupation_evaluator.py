...
class SeatOccupationEvaluator():
    ...
    def violation_and_cost(self, tutorial_occupation):
        ...
        theta = lambda x: numpy.heaviside(x, 0).astype(int)
        occupation2 = numpy.sum(numpy.apply_along_axis(theta, 1, occupation1), axis=0)
        ...
        for date_index, period_index in product:
            excess = occupation2[date_index, period_index] - self.__array_size.seat_count()
