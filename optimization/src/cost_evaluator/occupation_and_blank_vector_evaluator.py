...
class OccupationAndBlankVectorEvaluator():
    def __init__(self, period_count, occupation_limit,
                 blank_limit, occupation_costs, blank_costs):
        ...
        for binary in range(2**period_count):
            ...
            for period_index in range(period_count):
                if (binary >> period_index & 0b1):
                    ...
                    blank_count_buf += 1
            if occupation_count > self.__occupation_limit:
                self.__violation_vector[binary] += occupation_count - \
                    self.__occupation_limit
            if blank_count > self.__blank_limit:
                self.__violation_vector[binary] += blank_count - \
                    self.__blank_limit
