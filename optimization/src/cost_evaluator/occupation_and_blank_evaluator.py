...
class OccupationAndBlankEvaluator():
    def __init__(self, array_size, student_optimization_rule, teacher_optimization_rule):
    ...
    def __teacher_violation_and_cost(self, tutorial_occupation, teacher_group_occupation):
        teacher_tutorial_occupation = (numpy.einsum('ijkml->jml', tutorial_occupation) > 0).astype(int)
        ...
        for teacher_index, date_index in product:
            ...
            [violation, cost] = self.__teacher_vector_evaluator.violation_and_cost(vector)
    ...
    def __student_violation_and_cost(self, tutorial_occupation, student_group_occupation):
        student_tutorial_occupation = (numpy.einsum('ikjml->iml', tutorial_occupation) > 0).astype(int)
        ...
        for student_index, date_index in product:
            ...
            [violation, cost] = self.__student_vector_evaluator.violation_and_cost(vector)
            violation_summation += violation
            cost_summation += cost
        return [violation_summation, cost_summation]

    def violation_and_cost(self, tutorial_occupation, teacher_group_occupation, student_group_occupation):
        teacher_violation_and_cost = self.__teacher_violation_and_cost(tutorial_occupation, teacher_group_occupation)
        student_violation_and_cost = self.__student_violation_and_cost(tutorial_occupation, student_group_occupation)
        return [teacher + student for teacher, student in zip(teacher_violation_and_cost, student_violation_and_cost)] 
