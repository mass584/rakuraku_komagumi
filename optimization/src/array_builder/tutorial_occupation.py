...
class TutorialOccupation():
    ...
    def __build_tutorial_occupation(self):
        ...
        for tutorial_piece in tutorial_pieces:
            student_index = get_student_index(self.__term['term_students'], tutorial_piece['term_student_id'])
            teacher_index = get_teacher_index(self.__term['term_teachers'], tutorial_piece['term_teacher_id'])
            tutorial_index = get_tutorial_index(self.__term['term_tutorials'], tutorial_piece['term_tutorial_id'])
            ...
            if is_fixed: self.__fixed_tutorial_occupation[
                student_index, teacher_index, tutorial_index, date_index, period_index
            ] = 1
            if not is_fixed: self.__floated_tutorial_occupation[
                student_index, teacher_index, tutorial_index, date_index, period_index
            ] = 1
