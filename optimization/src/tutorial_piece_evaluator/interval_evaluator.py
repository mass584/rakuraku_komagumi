...
class IntervalEvaluator():
    ...
    def get_violation_and_cost_array(self, tutorial_occupation, cost_array):
        array = numpy.einsum('ijkml->ikml', tutorial_occupation)
        student_index_list = range(self.__array_size.student_count())
        tutorial_index_list = range(self.__array_size.tutorial_count())
        student_and_tutorial_index_list = itertools.product(
            student_index_list, tutorial_index_list)
        for student_index, tutorial_index in student_and_tutorial_index_list:
            cost = self.__each_interval_evaluator(
