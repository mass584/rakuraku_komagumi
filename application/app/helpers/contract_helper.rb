module ContractHelper
  ...
  def select_tag_teacher_term_id(contract)
    teachers = contract.term.teachers.joins(:subjects).where('subjects.id': contract.subject_term.subject.id)
                       .or(contract.term.teachers.joins(:subjects).where(id: contract.count))
                       .distinct
    options = content_tag(:option, '', 'value' => nil) +
              options_from_collection_for_select(teachers, :id, :name, contract.teacher_term_id)
    select_tag(
      :teacher_term_id,
      options,
