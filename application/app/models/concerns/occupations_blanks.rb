module OccupationsBlanks
  def daily_occupations_from(term, tutorials, groups)
    merge_tutorials_and_groups(term, tutorials, groups).sort.to_h.values.reduce(0) do |accu, occupied|
  ...
  def daily_blanks_from(term, tutorials, groups)
    ...
    result = merge_tutorials_and_groups(term, tutorials, groups).sort.to_h.values.reduce(init) do |accu, occupied|
      {
