class ScheduleController < ApplicationController
  ...
  def timetables
    ...
    @timetables = schedulemaster.timetables.where(
      scheduledate: schedulemaster.date_array_one_week(week),
    ).order(:scheduledate, :classnumber)
    @timetables
  end
  ...
  def get_data_for_front(schedulemaster)
    gon.teacher_groups = schedulemaster.subject_schedulemaster_mappings.map { |map|
      {
        subject_id: map.subject_id,
        teacher_id: map.teacher_id,
      }
    }
    gon.student_groups = schedulemaster.classnumbers.joins(:subject).where('subjects.classtype': '集団授業').map { |classnumber|
      {
