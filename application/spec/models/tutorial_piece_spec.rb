...
RSpec.describe TutorialPiece, type: :model do
  ...
  describe '担任講師バリデーションの検証' do
    before :each do
      ...
      tutorial_contract = term.tutorial_contracts.find_by(term_student_id: term_student.id)
      tutorial_contract.update(piece_count: 1, term_teacher: term_teacher_first)
      @seat = term.seats.joins(:timetable).where('timetables.date_index': 1,
                                                 'timetables.period_index': 1).first
      @seat.update(term_teacher: term_teacher_second)
      @tutorial_piece = tutorial_contract.tutorial_pieces.first
    end

    it '担当講師と座席の講師が一致しない場合にupdate失敗' do
      expect(@tutorial_piece.update(seat: @seat)).to eq(false)
      expect(@tutorial_piece.reload.seat_id).to eq(nil)
      expect(@tutorial_piece.errors.full_messages).to include('座席に割り当てられた講師と担当講師が一致しません')
    end
  end
  ...
  describe '１日のコマ数上限バリデーションの検証' do
    ...
    context '座席の新規設定時(seat_id : nil -> integer)' do
      it '最大コマ数を越した場合にupdate失敗' do
        ...
        expect(@fourth.errors.full_messages).to include('生徒の１日の最大コマ数を超えています')
      end
    end
